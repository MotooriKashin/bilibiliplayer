...
export class ScriptUtils {
    constructor(public _timer: TimerRuntime, protected _scriptManager: ScriptManager) { }
    /** 启动时间 */
    ...
    interval = (
        ...
        if (repeatCount === 0) {
            return this._timer.setInterval(callback, interval);
        }
        const ivl = this._timer.setInterval(() => {
            repeatCount--;
            if (repeatCount < 0) {
                this._timer.clearInterval(ivl);
            } else {
                callback();
            }
        }, interval);
