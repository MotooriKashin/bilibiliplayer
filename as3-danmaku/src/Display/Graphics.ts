...
export class Graphics {
    ...
    protected _callDrawMethod(method: string, params: any) {
        this.parent[<'addChild'>method](params);
    }
    ...
    public beginGradientFill(fillType: string,
        ...
        debug.warn('Graphics.beginGradientFill still needs work.');
        if (fillType !== 'linear' && fillType !== 'radial') {
            debug.warn('Graphics.beginGradientFill unsupported fill type : ' + fillType);
            return;
        }
    ...
    public beginShaderFill(shader: any, matrix: Matrix) {
        debug.warn('Graphics.beginShaderFill not supported.');
    }
    ...
    public drawTriangles(verts: Array<number>,
        ...
        if (indices.length % 3 !== 0) {
            debug.error('Graphics.drawTriangles malformed indices count. ' +
