...
interface IReportInfoInterface {
    template?: JQuery;
    showTextArea?: boolean;
    reportType?: number;
    reportList?: string;
    ...
    success_box?: JQuery;
    dmid?: string;
    show?: (danmaku: IDanmakuInterface, container?: JQuery) => void;
    hide?: (fromShow?: boolean) => void;
...
class Report {
    ...
    postReport(data: any, callback: Function) {
        this.player.directiveManager.sender(WD.DR_SEND_REPORT, data, (received?: IReceivedInterface) => {
    ...
    private reportInit() {
        ...
        this.report = {
            ...
            show: (danmaku: IDanmakuInterface, container?: JQuery) => {
                this.report.hide!(true);
                this.reportInit();
        ...
        const submitBtn = new Button(this.report.template!.find(`.${prefix}-danmaku-report-submit`), {
            ...
            click: () => {
                if (that.report.reportType) {
                    const data = {
                        // 'reportToAdmin': (that.report.important() ? 1 : 0),
                    ...
                    const callback = (defText: DefText, resp: any) => {
                        ...
                        switch (that.config.theme) {
                            case 'white':
                                new Tooltip({
                                    name: 'response',
                                    target: that.player.template.auxiliaryArea,
